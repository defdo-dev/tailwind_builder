defmodule Defdo.TailwindBuilder.VersionFetcherTest do
  ...
  describe "get_latest_npm_version/1" do
    test "returns error for unsupported package" do
      assert {:error, :package_not_supported} = TailwindBuilder.get_latest_npm_version("unsupported-package")
    end
  ...
  describe "checksum calculation" do
    ...
    test "handles invalid version format for checksum calculation" do
      ...
      assert {:error, :invalid_url} = TailwindBuilder.calculate_tailwind_checksum("invalid-version")
      assert {:error, :invalid_url} = TailwindBuilder.calculate_tailwind_checksum("../../../etc/passwd")
    end
  end
  ...
  describe "package validation" do
    test "validates package name input" do
      ...
      for name <- valid_names do
        ...
        refute name =~ ~r/[<>|&;`$()]/  # No shell injection chars
      end
    end
