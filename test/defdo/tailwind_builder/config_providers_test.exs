defmodule Defdo.TailwindBuilder.ConfigProvidersTest do
  ...
  describe "DevelopmentConfigProvider" do
    ...
    test "has fast operation limits" do
      ...
      assert limits.download_timeout == 30_000  # 30 seconds
      assert limits.build_timeout == 120_000    # 2 minutes
    ...
    test "has development-specific configurations" do
      ...
      assert cache.cache_builds == false  # Don't cache builds in development
      assert cache.auto_invalidate == true
    end

    test "validates policies permissively" do
      # Should allow all downloads
      assert DevelopmentConfigProvider.validate_operation_policy(:download, %{version: "3.4.17"}) == :ok
      assert DevelopmentConfigProvider.validate_operation_policy(:download, %{version: "unknown"}) == :ok
      
      # Should allow all builds
      assert DevelopmentConfigProvider.validate_operation_policy(:build, %{version: "4.1.11"}) == :ok
    end
  ...
  describe "ProductionConfigProvider" do
    ...
    test "has conservative operation limits" do
      ...
      assert limits.download_timeout == 120_000   # 2 minutes
      assert limits.build_timeout == 600_000      # 10 minutes
    ...
    test "validates policies strictly" do
      ...
      assert ProductionConfigProvider.validate_operation_policy(:download, %{version: "3.4.17"}) == :ok
  ...
  describe "StagingConfigProvider" do
    ...
    test "has balanced operation limits" do
      ...
      assert limits.download_timeout == 90_000    # 1.5 minutes
      assert limits.build_timeout == 300_000      # 5 minutes
    ...
    test "validates policies moderately" do
      ...
      assert StagingConfigProvider.validate_operation_policy(:download, %{version: "3.4.17"}) == :ok
  ...
  describe "TestingConfigProvider" do
    test "has minimal plugin support for testing" do
      ...
      assert is_map(plugins)
      assert Map.has_key?(plugins, "daisyui")
      assert Map.has_key?(plugins, "test-plugin")  # Special test plugin
    end
    ...
    test "has fast operation limits for quick testing" do
      ...
      assert limits.download_timeout == 10_000    # 10 seconds
      assert limits.build_timeout == 30_000       # 30 seconds
