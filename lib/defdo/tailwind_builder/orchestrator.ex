defmodule Defdo.TailwindBuilder.Orchestrator do
  ...
  def execute_workflow(opts \\ []) do
    ...
    with {:validate_download_policy, :ok} <- {:validate_download_policy, maybe_validate_policy(config_provider, version, :download)},
         {:validate_deploy_policy, :ok} <- {:validate_deploy_policy, maybe_validate_deploy_policy(opts, config_provider, version)},
         {:download, :ok} <- {:download, maybe_download(opts, source_path, version, config_provider)},
         {:plugins, plugin_results} <- {:plugins, maybe_apply_plugins(opts, plugins, version, source_path)},
         {:finalize, final_result} <- {:finalize, finalize_workflow_result(result, plugin_results, start_time, opts)} do
      {:ok, final_result}
    else
      {step, error} ->
  ...
  def build_and_deploy(opts \\ []) do
    ...
    with {:validate_policy, :ok} <- {:validate_policy, validate_build_policy(version, plugins, config_provider)},
         {:download, {:ok, download_result}} <- {:download, download_tailwind(version, config_provider)},
         {:plugins, {:ok, plugin_results}} <- {:plugins, apply_plugins(plugins, version, download_result.destination, config_provider)},
         {:build, {:ok, build_result}} <- {:build, compile_tailwind(version, download_result.destination, debug)},
         {:deploy, deploy_result} <- {:deploy, maybe_deploy(build_result, version, opts[:destination], config_provider, skip_deploy)} do
  ...
  def download_only(version, opts \\ []) do
    ...
    with {:validate_policy, :ok} <- {:validate_policy, validate_download_policy(version, config_provider)},
         {:get_checksum, checksum} <- {:get_checksum, get_expected_checksum(version, config_provider)},
         {:download, result} <- {:download, Downloader.download_and_extract([
           version: version,
  ...
  defp apply_plugins(plugins, version, source_path, config_provider) do
    ...
    results = for plugin_name <- plugins do
  ...
  defp get_policy_info(version, config_provider) do
    %{
      ...
      deployment_targets: [:r2, :s3]  # Could be dynamic
    }
  end
  ...
  defp finalize_workflow_result(base_result, {plugins_applied, plugin_errors}, start_time, opts) do
    ...
    result = if opts[:collect_metrics] do
      Map.put(result, :metrics, %{
        total_time: total_time,
        download_time: total_time, # Simplified for now
        plugin_time: 0
      })
    else
      result
    ...
    result = if opts[:validate_at_each_step] do
    ...
    result = if opts[:generate_reports] do
