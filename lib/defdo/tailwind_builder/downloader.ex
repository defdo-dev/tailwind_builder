defmodule Defdo.TailwindBuilder.Downloader do
  ...
  defp do_download_and_extract(opts) do
    ...
    Telemetry.track_event(:download, :start, %{
      version: version,
      url: url,
    ...
    with {:validate_url, :ok} <-
           {:validate_url, maybe_validate_url_with_telemetry(url, validate_url)},
         {:download, {:ok, content}} <-
           {:download, download_content_with_telemetry(url, version)},
         {:validate_checksum, :ok} <-
           {:validate_checksum,
      ...
      Telemetry.track_event(:download, :success, %{
        version: version,
        size: byte_size(content),
        duration_ms: duration_ms
      })
      ...
      {step, error} ->
        ...
        Telemetry.track_event(:download, :error, %{
          version: version,
          step: step,
          error: inspect(error),
          duration_ms: duration_ms
  ...
  def validate_checksum(content, expected_checksum)
      when is_binary(content) and is_binary(expected_checksum) do
  ...
  def validate_github_url(url) when is_binary(url) do
    pattern =
      ~r{^https://github\.com/tailwindlabs/tailwindcss/archive/refs/tags/v\d+\.\d+\.\d+\.tar\.gz$}
  ...
  def get_download_info(content) when is_binary(content) do
    ...
    %{
      ...
      # 100KB - 200MB
      is_reasonable_size: size >= 100_000 and size <= 200_000_000
    }
  end
  ...
  defp download_content_with_telemetry(url, version) do
    ...
    case result do
      {:ok, content} ->
        ...
        # Assume cache miss for HTTP requests
        Metrics.record_cache_metrics(:download, url, :miss)
  ...
  defp maybe_validate_checksum_with_telemetry(content, version, expected_checksum) do
    ...
    Telemetry.track_event(:download, :checksum_validation_start, %{
      version: version,
      has_expected: expected_checksum != nil
    })
    ...
    case result do
      :ok ->
        Telemetry.track_event(:download, :checksum_validation_success, %{
          version: version,
          duration_ms: duration_ms
        })

      error ->
        Telemetry.track_event(:download, :checksum_validation_error, %{
          version: version,
          error: inspect(error),
          duration_ms: duration_ms
        })
  ...
  defp save_content_with_telemetry(content, destination, version) do
    ...
    Telemetry.track_event(:download, :file_save_start, %{
      destination: destination,
      version: version,
      size_bytes: size_bytes
    })
  ...
  defp extract_tar_with_telemetry(tar_path) do
    ...
    Telemetry.track_event(:download, :extraction_start, %{
      tar_path: tar_path,
      file_size: file_size
    })
